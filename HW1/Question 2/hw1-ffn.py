...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        self.output = self.output = nn.Linear(hidden_size if layers > 0 
...
def predict(model, X):
    ...
    model.eval()
    y_hat = model(X)
    _, preds = torch.max(y_hat, dim=1) #index of highest score across row (index of class)
    return preds
...
def plot(epochs, plottables, filename=None, ylim=None):
    ...
    for label, plottable in plottables.items():
        plot_epochs = torch.cat([torch.tensor([0]), epochs]) # match length (include epoch 0)
...
def main():
    ...
    plot(epochs, losses, filename=f'training-loss-{config}.pdf')
    print(f"Final Training Accuracy: {train_accs[-1]:.4f}")
    print(f"Best Validation Accuracy: {max(valid_accs):.4f}")
    val_accuracy = { "Valid Accuracy": valid_accs }
    plot(epochs, val_accuracy, filename=f'validation-accuracy-{config}.pdf')

    with open("results.csv", "a") as f:
        f.write(f"{opt.hidden_size},{opt.layers},{opt.learning_rate},{opt.l2_decay},{opt.dropout},{max(valid_accs)},{test_acc}\n")
